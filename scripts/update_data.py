...
def scrape_draws(page):
    ...
    for row in page.locator("table tr").all():
        ...
        if len(nums) != 6 or len(set(nums)) != 6:
            continue
        draws.append({"numero_concorso": int(m.group(1)), "descrizione": cells[0], "numeri": sorted(nums)})
...
def main():
    ...
    data = {
        ...
        "stats": stats,
        "ultime_estrazioni": [{"descrizione": d["descrizione"], "numeri": d["numeri"]} for d in draws],
    }
